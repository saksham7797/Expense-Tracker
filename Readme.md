...
src/main/java/com/saksham_kumar/
...
(All endpoints below require header: Authorization: Bearer <your_token>)
...
/expenses?type=category&value=Food

Get expenses filtered by Category

GET

/expenses?type=filter&value=week
...
Update an expense (Price, Desc, etc.)
...
Register: Send a POST to /request with JSON body {"userName": "admin", "password": "123"}.

Login: Send a POST to /login with the same credentials.

Get Token: Copy the token string from the response.

Authorize: For all subsequent requests, go to the Headers tab in Postman and add:

Key: Authorization

Value: Bearer <paste_your_token_here>

Operations: Now you can Add, List, or Delete expenses securely.
